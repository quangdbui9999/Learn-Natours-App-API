...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 2. Check if the user is exists, password is correct
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError('User recently changed password@ Plase login in again!', 401)
    );
  }
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    // Roles is an array: ['admin', 'lead-guide']. Roles: 'user' (default)
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
  // User.findByIdAndUpdate() does not work for UPDATE
